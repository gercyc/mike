...
CREATE TABLE "document_edits" (
	...
	CONSTRAINT "document_edits_status_check" CHECK ("document_edits"."status" IN ('pending','accepted','rejected'))
);
--> statement-breakpoint
CREATE TABLE "document_versions" (
	...
	"display_name" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "document_versions_source_check" CHECK ("document_versions"."source" IN ('upload','user_upload','assistant_edit','user_accept','user_reject','generated'))
);
--> statement-breakpoint
...
CREATE TABLE "user_api_keys" (
	...
	CONSTRAINT "user_api_keys_provider_check" CHECK ("user_api_keys"."provider" IN ('claude', 'gemini', 'openai', 'openrouter'))
);
--> statement-breakpoint
...
CREATE INDEX "idx_users_verification_token" ON "users" USING btree ("email_verification_token");--> statement-breakpoint
