// Compact model picker for the add-in composer. Mirrors the desktop's
// frontend/src/app/components/assistant/ModelToggle.tsx — same model IDs and
// labels — but uses a self-contained dropdown (no headless-ui) since the
// task pane is narrow.
  ...
  useEffect(() => {
    ...
    getAiKeys()
      .then((keys) => {
        if (!alive) return;
        setAvail({
          anthropic: !!keys.anthropic?.enabled && !!keys.anthropic?.key,
                ...
                {items.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`flex-1 truncate ${ok ? "text-gray-700" : "text-gray-400"}`}
                      >
                        {m.label}
                      </span>
                      {isSelected && ok && (
                        <span className="text-mike-500 text-[10px]">✓</span>
                      )}
                    </button>
                  );
                })}
