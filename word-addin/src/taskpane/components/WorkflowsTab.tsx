...
export default function WorkflowsTab() {
  ...
  function handleUseInChat(wf: { id: string; title: string }) {
    ...
    // Fallback toast in case the tab-switch listener isn't wired yet.
    setToast("Workflow ready — switch to Chat tab to use it.");
    ...
    return (
      <div className="flex flex-col h-full overflow-y-auto relative">
        ...
        <div className="px-4 py-2">
          <h2 className="font-serif text-xl text-gray-900">{open.title}</h2>
          {open.practice ? (
            <div className="text-xs text-gray-500 mt-0.5">{open.practice}</div>
          ) : null}
        </div>
        <div className="px-3 pb-2">
          <button
            onClick={() => handleUseInChat({ id: open.id, title: open.title })}
        ...
        <div className="px-4 py-2 text-sm text-gray-800 prose prose-sm max-w-none">
          {open.prompt_md ? (
            ...
            <div className="text-xs text-gray-400">No prompt body.</div>
          )}
        </div>
