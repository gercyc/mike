...
export default function McpPage() {
  const t = useTranslations("account.mcp");
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h2 className="text-2xl font-medium font-serif mb-1">{t("title")}</h2>
      ...
      <div className="rounded-lg border border-gray-200 p-4 space-y-3">
        <div className="font-medium text-sm">{t("issueNewToken")}</div>
        <div className="flex items-center gap-2">
          <Input
          ...
          <Button onClick={create} disabled={creating || !label.trim()}>
            {creating ? (
              ...
              <>
                <Plus className="w-4 h-4 mr-1" /> {t("issue")}
              </>
            )}
          </Button>
        </div>
      ...
      <div className="space-y-2">
        <div className="font-medium text-sm">{t("activeTokens")}</div>
        {tokens.length === 0 ? (
          <div className="text-xs text-gray-500 italic">{t("noTokens")}</div>
        ) : (
          <div className="space-y-2">
            {tokens.map((tok) => (
              <div
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="font-medium truncate">{tok.label}</div>
                  <div className="text-xs text-gray-500">
                    <span className="font-mono">…{tok.preview}</span>
                    {" · "}
                    {tok.scope}
                    {" · "}
                    {tok.last_used_at
